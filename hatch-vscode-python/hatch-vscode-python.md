...
В папке `%LOCALAPPDATA%\hatch\env\virtual` находим папку с нашим виртуальным окружением и спускаемся в папку `Scripts`, полный путь которой и копируем:

![Путь с Python интерпретатором](img/environment_03.png)

_Рисунок 18 — Путь с Python интерпретатором_

Получившийся путь вставляем в VSCode:
...
Как добавляются библиотеки и как происходит их установка? В отличии от других систем, тут не нужно в консоли прописывать команду наподобие 
