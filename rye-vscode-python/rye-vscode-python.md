...
То есть папки `.pytest_cache`, `.ruff_cache`, `.venv` вам не нужны. Они нужны конкретно вашему локальной версии проекта. Если вы используете Git, то благодаря файлу `.gitignore` лишние папки не попадут в переносимую папку.
...
Пишем команду для создания виртуального окружения с теми же самыми библиотеками, что были в проекте. Это прописано в файлах
...
Выбираем в VSCode виртуальное окружение из папки `.venv` (VSCode скорее всего даже предложить поменять виртуальное окружение самостоятельно).
...
В статье [](https://github.com/Harrix/harrix.dev-articles-2024/blob/main/-/-.md) | [🡥](https://harrix.dev/ru/articles/2024/-/)